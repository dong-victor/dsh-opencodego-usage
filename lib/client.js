...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var STYLE_TEXT = [
      ...
      '.og-entry{container:og-entry/inline-size}',
      '.og-entryCompact{display:none;font-size:11px;font-weight:500;color:var(--dsw-alias-label-primary);font-variant-numeric:tabular-nums}',
      '.og-entryCompact.hot{color:var(--dsw-alias-state-warn-primary)}',
      '.og-entryCompact.danger{color:var(--dsw-alias-state-error-primary)}',
      '@container og-entry (max-width: 140px){.og-entryHead{gap:0}.og-entryIcon{display:none !important}.og-entryLabel{display:none !important}.og-windowRows{display:none !important}.og-entryCompact{display:inline !important}}',
      '@container og-entry (min-width: 141px){.og-entryCompact{display:none !important}}',
    ].join('')
    ...
    function createEntry(controller) {
      ...
      entry.innerHTML =
        '<span class="og-entryHead">' +
          ...
          '<span class="og-entryCompact">--</span>' +
        '</span>' +
    ...
    function renderEntrySummary(entry, body) {
      ...
      var compactPct = entry.querySelector('.og-entryCompact')
      ...
      for (var i = 0; i < WINDOWS.length; i++) {
        ...
        if (windowData !== undefined && typeof windowData.percent === 'number') {
          ...
          compactValues.push({ key: def.key, percent: percent, severity: severity })
        } else {
          fill.style.width = '0%'
      ...
      if (compactPct && compactValues.length > 0) {
        var monthly = compactValues[2] || compactValues[0]
        compactPct.textContent = monthly.percent + '%'
        compactPct.className = 'og-entryCompact' + (monthly.severity !== '' ? ' ' + monthly.severity : '')
        compactPct.title = tt('sidebar.monthly') + ' ' + monthly.percent + '%'
      } else if (compactPct) {
        compactPct.textContent = '--'
        compactPct.className = 'og-entryCompact'
        compactPct.title = ''
      }
