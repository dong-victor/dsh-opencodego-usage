 ...
 * derived from the same wiring the LLM provider already uses (the
 * `llm-deepseek` settings namespace — `baseURL` https://opencode.ai/zen/go/v1,
 * `apiKeyEnv` OPENCODE_GO_API_KEY stored through the credentials service), with
 * config overrides available for other zen deployments.
 *
...
/** Settings namespace whose wiring this plugin reuses (spelled, not imported: the browser half must not depend on Host packages). */
...
export const OPENCODE_GO_GUIDANCE =
  '本机已安装 @dong-victor/dsh-opencodego-usage 插件（查看链接的 opencode Go 套餐余额）：侧边栏「Go 余额」入口。能力：查询 GET https://opencode.ai/zen/go/v1/usage（Authorization: Bearer），显示 rolling / weekly / monthly 三档用量百分比、状态与重置时间；提供 opencode_go_balance 工具供 agent 查询，宿主侧带 30s 缓存；密钥与端点默认复用 DSH 已配置的 llm-deepseek 接线（credentials 服务 + settings），无需二次配置。限制：余额查询依赖 opencode 官方端点，网络不可达、密钥失效（401）或无 Go 套餐时返回错误；密钥仅在本机使用、不会外发；缓存期内重复查询不会触发上游请求。用户提到「opencode go 套餐 / Go 余额 / 用量 / balance / 额度」时即指本插件，请据此协作。'
 ...
 * @property {string} [apiBaseUrl] API root override, e.g. "https://opencode.ai" or a full base URL like "https://opencode.ai/zen/go/v1". Default: derived from llm-deepseek baseURL.
 * @property {string} [apiKeyEnv] Credential reference override (e.g. "OPENCODE_GO_API_KEY"). Default: llm-deepseek apiKeyEnv.
 ...
 * plugin config and the running tree. The default follows the llm-deepseek
 * wiring: its settings namespace supplies baseURL and apiKeyEnv; the
 * credentials service resolves the ref; process.env is the fallback.
...
export async function resolveSource(ctx, config = {}) {
  ...
  const rawBase = config.apiBaseUrl ?? llm?.baseURL ?? ''
  const baseUrl = originOf(rawBase) || DEFAULT_BASE_URL
  const keyRef = credentialRef(config.apiKeyEnv ?? llm?.apiKeyEnv ?? DEFAULT_KEY_REF)
  let apiKey = ''
  try {
